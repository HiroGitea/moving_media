...
### 新增
- **储存卡自动检测**（`watcher.rs`）：后台线程每 2 秒轮询系统卷，插入储存卡后自动扫描并对比数据库，顶部横幅提示备份状态；支持 macOS（`/Volumes/`）、Linux（`/media/`）、Windows（盘符）
...
- **Panic 钩子**：未捕获异常写入 `~/.local/share/moving_media/crash.log`
